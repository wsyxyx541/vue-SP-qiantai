  ...
  <div class="detail">
    ...
    <section class="con">
      ...
      <el-breadcrumb separator="/">
        <el-breadcrumb-item v-show="categoryView.category1Name">{{
          categoryView.category1Name
        }}</el-breadcrumb-item>
        <el-breadcrumb-item v-show="categoryView.category2Name">{{
          categoryView.category2Name
        }}</el-breadcrumb-item>
        <el-breadcrumb-item v-show="categoryView.category3Name">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: item1.isChecked == 0 }"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前路由跳转，从A路由跳转到B路由，但是再加入购物车这里，进行路由跳转之前，
                我们要发送请求，把用户购买的产品的信息通过请求的方式通知服务器，
                服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果服务器的数据没有回来，skuInfo这个对象是空对象，传一个没有的参数，就返回的是undefined
      return this.skuInfo.skuImageList || [{}];
    },
  },
  methods: {
    //产品属性切换高亮
    changeActive(item1, arr) {
      //遍历全部的售卖高亮属性 全部置为1 没有高亮效果
      arr.forEach((ele) => {
        ele.isChecked = 1;
      });
      //点击的那个售卖高亮属性值为0
    ...
    async addShopCart() {
      ...
      let sku = { skuId: this.$route.params.skuid, skuNum: this.skuNum };
      /* 当前这里是派发一个action，也向服务器发送请求，判断加入购物车是成功了还是失败了，进行相应的操作。
      ...
      */
      try {
        //2.服务器存储成功---进行路由跳转
        await this.$store.dispatch("detail/addOrUpdateShopCart", sku);

        //进行路由跳转
        //在路由跳转的时候还需要将产品的信息带给下一级路由组件
        //下面这种手段路由跳转一级参数可以
        //在进行路由传参的时候，尽量传递一些简单的数据过去，
        //像skuInfo这种复杂的数据，最好是存储在会话存储里面，本地存储有点浪费空间了
        //不管是本地存储还是对话存储，一般存储的都是字符串
